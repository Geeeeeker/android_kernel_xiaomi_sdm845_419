		...
		uint32_t length)
{
	...
	pr_info("[ELUS] %s(): start (port=%d, param_id=0x%x, module_id=0x%x, len=%u)\n",
		__func__, port, param_id, module_id, length);

	port_id = q6audio_get_port_id(port);
	ret = q6audio_validate_port(port_id);
	if (ret < 0) {
		pr_err("[ELUS] invalid port id=0x%x ret=%d\n", port_id, ret);
		return -EINVAL;
	}
	...
	if (!packed_param_data) {
		pr_err("[ELUS] failed kzalloc packed_param_data\n");
		return -ENOMEM;
	}
	...
	if (ret) {
		pr_err("[ELUS] q6common_pack_pp_params() failed: %d\n", ret);
		goto fail_cmd;
	}
	...
	if (q6common_is_instance_id_supported()) {
		...
		if (!set_param_v3) {
			ret = -ENOMEM;
			pr_err("[ELUS] failed kzalloc set_param_v3\n");
			goto fail_cmd;
		}
		...
		memcpy(&set_param_v3->param_data, packed_param_data, packed_data_size);

		pr_info("[ELUS] sending V3 param cmd (port_id=0x%x, size=%u)\n",
			port_id, packed_data_size);
		...
		if (!set_param_v2) {
			ret = -ENOMEM;
			pr_err("[ELUS] failed kzalloc set_param_v2\n");
			goto fail_cmd;
		}
		...
		set_param_v2->apr_hdr.pkt_size =
			sizeof(struct afe_port_cmd_set_param_v2) + packed_data_size;
		...
		memcpy(&set_param_v2->param_data, packed_param_data, packed_data_size);

		pr_info("[ELUS] sending V2 param cmd (port_id=0x%x, size=%u)\n",
			port_id, packed_data_size);
		atomic_set(elus_afe.ptr_state, 1);
	...
	if (ret < 0) {
		pr_err("[ELUS] apr_send_pkt() failed ret=%d (port=%d param=0x%x)\n",
		       ret, port, param_id);
		goto fail_cmd;
	}

	pr_info("[ELUS] waiting for response (index=%d, timeout=%dms)\n",
		index, elus_afe.timeout_ms * 10);
	...
	if (!ret) {
		pr_err("[ELUS] wait_event_timeout() expired (port=%d param=0x%x)\n",
		       port, param_id);
		ret = -ETIMEDOUT;
		goto fail_cmd;
	}
	...
	if (atomic_read(elus_afe.ptr_status) != 0) {
		pr_err("[ELUS] set param cmd failed: ptr_status=%d\n",
		       atomic_read(elus_afe.ptr_status));
		ret = -EINVAL;
		goto fail_cmd;
	}

	pr_info("[ELUS] param 0x%x applied successfully (port=%d)\n", param_id, port);
	ret = 0;

fail_cmd:
	pr_debug("[ELUS] %s param_id=0x%x ret=%d\n", __func__, param_id, ret);
	kfree(set_param_v2);
	kfree(set_param_v3);
